...
poetry -C $RENGINE_FOLDER install --only dev --no-root
...
poetry -C $RENGINE_FOLDER run python3 manage.py collectstatic --noinput
...
watchmedo shell-command \
    ...
    --command="echo 'Collecting static files...' && poetry -C $RENGINE_FOLDER run python3 manage.py collectstatic --noinput" \
    --recursive \
    --wait \
...
watchmedo auto-restart \
    ...
    -- \
    poetry -C $RENGINE_FOLDER run daphne -b 0.0.0.0 -p 8000 --verbosity 2 reNgine.asgi:application
