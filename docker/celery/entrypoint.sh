...
poetry -C $RENGINE_FOLDER run python3 manage.py makemigrations
print_msg "Migrate database"
poetry -C $RENGINE_FOLDER run python3 manage.py migrate
print_msg "Collect static files"
poetry -C $RENGINE_FOLDER run python3 manage.py collectstatic --no-input --clear

# Load default engines, keywords, and external tools
print_msg "Load default engines"
poetry -C $RENGINE_FOLDER run python3 manage.py loaddefaultengines
print_msg "Load default keywords"
poetry -C $RENGINE_FOLDER run python3 manage.py loaddata fixtures/default_keywords.yaml --app scanEngine.InterestingLookupModel
print_msg "Load default external tools"
poetry -C $RENGINE_FOLDER run python3 manage.py loaddata fixtures/external_tools.yaml --app scanEngine.InstalledExternalTool
...
worker_command() {
    ...
    if [ "$CELERY_DEBUG" = "1" ]; then
        echo "watchmedo auto-restart --recursive --pattern=\"*.py\" --directory=\"$RENGINE_FOLDER\" -- \
            poetry -C $RENGINE_FOLDER run celery -A reNgine worker \
            --pool=solo \
            --loglevel=$CELERY_LOGLEVEL \
            -Q $queue -n $worker_name"
    else
        echo "poetry -C $RENGINE_FOLDER run celery -A reNgine worker \
            --pool=gevent \
            --loglevel=$CELERY_LOGLEVEL \
